...
def doHead(q):
    ...
    if velocity == "high":
        ...
        if L == None:
            SendData(round(602 * q7.get()), 500, 500, 1, 1)
        elif L == 'reduce_distance':
            L = None 
            SendData(round(602 * q7.get()), 500, 500, 1, 1)
...
def doBack(q):
    global tProcess 
    SendData(860, 400, 400, 0, 1)
    if q5.qsize() == 0: 
        q5.put("go cablib 1")
def doRotateLeft(q):
    global tProcess 
    tProcess = pCALIBDONE
    SendData(427, 400, 400, 0, 1)

def doRotateRight(q): 
    global tProcess
    tProcess = pCALIBDONE 
    SendData(427, 400, 400, 1, 0)
...
def doOpenCV(q, q2):
    ...
    while True:
        if q2.qsize() == 2:
            ...
            print(f"list_process_backward = {list_process_backward}")

            print("\ndo head number: ", count_before)
            print("do head number after rotate: ", count_after)
            q7.put(count_before)
            q7.put(count_after)
            q7.put(count_after)
            q7.put(count_before)
