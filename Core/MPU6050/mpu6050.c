...
void MPU6050_Read_All(I2C_HandleTypeDef *I2Cx, MPU6050_t *DataStruct)
{
    ...
    DataStruct->Gx = DataStruct->Gyro_X_RAW / 131.0;
    DataStruct->Gy = DataStruct->Gyro_Y_RAW / 131.0;
    DataStruct->Gz = DataStruct->Gyro_Z_RAW / 131.0 - 2.1065;
...
void kalman_single(KalmanSingle_t *Kalman, float measure_noise, float process_noise){
	float R = measure_noise*measure_noise;
	float Q = process_noise*process_noise;
	float P_, K;
	// Kalman
	P_ = Kalman->P + Q;
	K = P_ / (P_ + R);
	Kalman->x_hat = Kalman->x_hat + K * (Kalman->x - Kalman->x_hat) ;
	Kalman->P = (1 - K) * P_;
}
